...
def parse_feed(xml: str) -> list[dict]:
    ...
    for raw in xml.split("<item")[1:]:
        ...
        desc = xml_match(block, r"<description>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?</description>")
        pub = xml_match(block, r"<pubDate>([^<]+)</pubDate>")
        if title and link:
            out.append({"id": guid, "title": title, "link": link, "summary": desc, "published": pub})
    if out:
        return out
    # ── Atom fallback ─────────────────────────────────────────────────────────
    for raw in xml.split("<entry")[1:]:
        ...
        title = re.sub(r"\s+", " ", xml_match(block, r"<title[^>]*>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?</title>"))
        link = xml_match(block, r'<link[^>]*href="([^"]+)"')
        entry_id = xml_match(block, r"<id>([^<]+)</id>") or link
        summary = xml_match(block, r"<(?:summary|content)[^>]*>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?</(?:summary|content)>")
        updated = xml_match(block, r"<(?:updated|published)>([^<]+)</(?:updated|published)>")
        if title and link:
            out.append({"id": entry_id, "title": title, "link": link, "summary": summary, "published": updated})
    return out
...
def main():
    ...
    for feed in feeds:
        try:
            xml = safe_fetch_text(feed["url"], timeout_ms=15_000, max_bytes=4 * 1024 * 1024)
